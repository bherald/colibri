...
typedef struct { int layer, expert; uint32_t count; } PinRec;

static int pin_cmp(const void *a, const void *b){
    const PinRec *x=a, *y=b;
...
int main(int argc, char **argv){
    ...
    unsigned char *pinmap=NULL;
    int pin_layers=maxlayer+1, selected_pins=0;
    if(pin_usage){
        FILE *pf=fopen(pin_usage,"r");
        if(!pf){ perror(pin_usage); return 1; }
        PinRec *records=NULL; size_t nr=0, cr=0;
        int l,e, declared_layers=0, declared_experts=0; uint32_t count;
        while(fscanf(pf,"%d %d %u",&l,&e,&count)==3){
            if(l==-1){ declared_layers=e+1; declared_experts=(int)count; continue; }
            if(l<0 || e<0 || !count) continue;
            if(nr==cr){ cr=cr?cr*2:1024; records=xrealloc(records,cr*sizeof *records); }
            records[nr++]=(PinRec){l,e,count};
            if(l+1>pin_layers) pin_layers=l+1;
            if(e+1>nexp) nexp=e+1;
        }
        fclose(pf);
        if(declared_layers>pin_layers) pin_layers=declared_layers;
        if(declared_experts>nexp) nexp=declared_experts;
        pinmap=calloc((size_t)pin_layers*(size_t)nexp,1);
        if(!pinmap){ fprintf(stderr,"oom\n"); return 1; }
        qsort(records,nr,sizeof *records,pin_cmp);
        for(size_t i=0;i<nr && selected_pins<pin_count;i++){
            PinRec *r=&records[i];
            if(r->layer>=pin_layers || r->expert>=nexp) continue;
            size_t ix=(size_t)r->layer*(size_t)nexp+(size_t)r->expert;
            if(!pinmap[ix]){ pinmap[ix]=1; selected_pins++; }
        }
        free(records);
        if(selected_pins<pin_count)
            fprintf(stderr,"pin usage contains only %d selectable experts (requested %d)\n",
                    selected_pins,pin_count);
    }
    ...
    for(int l=0;l<=maxlayer;l++){
        LT *t=&lt[l];
        if(!t->nr) continue;
        /* Pins are either the runtime's exact global history selection or the
         * legacy per-layer approximation selected from this trace. */
        memset(cnt,0,nexp*sizeof *cnt);
        for(size_t i=0;i<t->nr;i++) cnt[t->routed[i]]++;
        memset(pinned,0,nexp);
        if(pinmap && l<pin_layers)
            memcpy(pinned,pinmap+(size_t)l*(size_t)nexp,(size_t)nexp);
        else for(int a=0;a<npin;a++){ int best=-1;
